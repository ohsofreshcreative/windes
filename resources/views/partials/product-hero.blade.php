...
<section data-gsap-anim="section"
  class="hero-sub relative {{ $sectionClass }}"
...
@endif
      ...
      {{-- Tytuł produktu jako H1 (WooCommerce domyślnie renderuje go w summary, ale tu damy własny jeśli chcesz nadpisać) --}}
      <h1 class="text-white m-header">{{ get_the_title() }}</h1>

      {{-- Przyciski / CTA z ACF link --}}
      @if(!empty($link['url']))
